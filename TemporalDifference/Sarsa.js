...
for (let episode = 0; episode < EPISODES; episode++) {
  ...
  while (state !== END_STATE) {
    ...
    // Sarsa核心：选择下一个动作（同样用ε-贪心）
    let nextAction;
    if (nextState === END_STATE) {
      // 到达终点，没有下一个动作
      nextAction = -1;
    } else if (randomUniform() < EPSILON) {
      nextAction = randomChoice(ACTION_NUM);
    } else {
      nextAction = arrayArgMax(qTable[nextState]);
    }
    
    // Sarsa Q值更新公式（核心区别）
    let target;
    if (nextState === END_STATE) {
      // 终点状态没有下一个动作，目标值只有即时奖励
      target = reward;
    } else {
      // 用实际选择的下一个动作的Q值，而非最大值
      target = reward + GAMMA * qTable[nextState][nextAction];
    }
    qTable[state][action] += ALPHA * (target - qTable[state][action]);
    
    // 状态和动作都转移（Sarsa需要同时更新状态和动作）
    state = nextState;
    action = nextAction;
  }
}
